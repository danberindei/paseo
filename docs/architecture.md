...
> **Daemon start is main-process single-flight.** Every window mounts its own copy of the app and asks main to start the managed daemon, so start requests arrive once per window, not once per launch. `startDaemonOnce()` in `daemon/daemon-manager.ts` coalesces concurrent requests into one operation; without it two windows each ran the whole status/stop/spawn sequence, stopped the same daemon twice on a version mismatch, and spawned replacements that raced for the port, leaving the losing window reporting a start failure for a daemon that is running. A renderer-side singleton cannot coordinate this — it cannot see the other window. A restart deliberately does not join an in-flight start: it waits for that start to settle, then stops and starts fresh.
>
